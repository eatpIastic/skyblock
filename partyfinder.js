...
register("step", () => {
    if(!World.isLoaded() || Player?.getContainer()?.getName() != "Party Finder") {
        if(Object.keys(partyFinderList).length !== 0) {
            partyFinderList = {};
        }
...
const updateList = () => {
    ...
    let tempPartyFinderList = {};

    for (let i = 10; i < container.getSize(); i++) {
        ...
        if(dungeonType == "master_catacombs" && [4, 6, 7].includes(floor) && !hasMissingClasses(lore)) {
            tempPartyFinderList[i] = getMissingClasses(lore).map(str => str[0]).join(' ');
        }
    }

    partyFinderList = tempPartyFinderList;
}

registerWhen(register("renderSlot", (slot, gui, event) => {
    if(Player.getContainer().getName() != "Party Finder") {
        return;
    }
    if(!partyFinderList[slot.getIndex()]) {
        return;
    }

    Renderer.translate(slot.getDisplayX(), slot.getDisplayY() - (slot.getIndex() % 2 == 0 ? 0 : 4), 275);
    Renderer.scale(0.5, 0.5);
    Renderer.drawString(`${partyFinderList[slot.getIndex()]}`, 0, 16, true);
}), () => Object.keys(partyFinderList).length !== 0);
